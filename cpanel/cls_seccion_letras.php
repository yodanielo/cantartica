...
class Registro extends MantixBase {
    ...
    function formulario() {
        if($_POST["accion"]!=20 && $_POST["accion"]!=2) {
            $_POST["accion"]=20;
            $_POST["idobj"]=$this->datos->get_simple("select id from can_secciones where nombre='Letras' and idnumero=".$_SESSION["num_activo"]);
        }
        if($_POST["accion"]=="2") {
    ...
    function lista() {
        $r = new MantixGrid();
        $sql="select *, replace(replace(replace(plantilla,'plantilla','Plantilla '),4,1),5,4) as pl from can_secciones where nombre like 'Letras' and idnumero=".$_SESSION["num_activo"];
        $r->atributos=array("sql"=>$sql,"nropag"=>"20","ordenar"=>"id desc","url_form"=>"seccion_letras.php","url"=>"seccion_letras.php","ver_eliminar"=>"0","ver_estado"=>"0","ver_nro"=>"0");
        $r->columnas=array(
    ...
    function pre_upd() {
        $this->datos->agregar("nombre","Letras");
