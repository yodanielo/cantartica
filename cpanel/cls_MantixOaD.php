...
//	$this->db =  mysql_select_db("cantartica", $this->con);
    }

    function ejecutar($sql) {
        $res= mysql_query(utf8_decode($sql),$this->con);
        if(mysql_error())
            ...
            if ($this->cols[$a]->nombre == $nombre) {
                switch($this->cols[$a]->tipo) {
                    ...
                    case "datetime":
                        ...
                        break;
                    default:  $v = htmlspecialchars($this->cols[$a]->valor);
                        break;
                }
            }
        }
    ...
    function agregar( $nombre, $valor ) {
        for ($a=1;$a<=$this->total;$a++) {
            if ($this->cols[$a]->nombre == $nombre) {
                $this->cols[$a]->valor =utf8_decode($this->normalizar($valor));
    ...
    function agregar_todo() {
        for ($a=1;$a<=$this->total;$a++) {
            if(isset($_POST[$this->cols[$a]->nombre])) {
    ...
    function get_fila($idv) {
        ...
        if ($this->rs) {
            if (mysql_num_rows($this->rs)>0) {
                ...
                for ($a=1;$a<=$this->total;$a++) {
                    $this->cols[$a] = new Columna(mysql_field_name($this->rs,$a-1),mysql_field_type($this->rs,$a-1),utf8_encode($fila[$a-1]));
                }
            }
        }
        else {
    ...
    function toggle_estado(){
        $r = $this->ejecutar("update ".$this->tabla." set estado=if(estado=1,0,1) where id IN (" .implode(",",$_POST["cid"]).")");
        if (!$r) {
            $this->error = mysql_error($r);
            $this->c_error= 19;
            return -1;
    ...
    function lista($str_sql,$str_filtro, $str_orden,$p_actual ) {
        if($p_actual==null || $p_actual<=0 || $p_actual=="")
            $p_actual=1;
        $c_filtro="";
        $c_sql=$str_sql;
        $c_orden=$str_orden;
