...
class Usuarios extends MantixBase {
    ...
    function __construct() {
        $this->ini_datos("can_usuarios","usuario");
    }
    ...
    function ingresar() {
        $this->datos->listaSP("select * from can_usuarios where usuario='".str_replace("'","",trim($_POST["txt_user"]))."' and password='".md5(trim($_POST["txt_pass"]))."'","","");
    ...
    function modificar_password() {
        if($_SESSION["user"]["password"]==trim(md5($_POST["password"]))) {
            $this->datos->ejecutar("update can_usuarios set password='".md5(trim($_POST["nueva"]))."' where id=".$_SESSION["user"]["id"]);
    ...
    function lista() {
        $r = new MantixGrid();
        $r->atributos=array("tabla"=>"can_usuarios","nropag"=>"20","ordenar"=>"usuario");
        $r->columnas=array(
