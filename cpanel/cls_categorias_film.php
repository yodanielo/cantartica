...
class Registro extends MantixBase {
    ...
    function formulario() {
        $xx = $this->datos->get_simple("select plantilla from can_secciones where nombre like 'Film / Vídeo' and idnumero=" . $_SESSION["num_activo"]);
        $this->semuestra = $xx;
    ...
    function lista() {
        ...
        $sql = "SELECT can_categorias. *, replace(replace(replace(can_categorias.plantilla,'plantilla','Plantilla '),4,1),5,4) as pl
                ...
                where can_secciones.nombre like 'Film / Vídeo' and can_secciones.idnumero=" . $_SESSION["num_activo"];
        $r->atributos = array("sql" => $sql, "nropag" => "20", "ordenar" => "posicion", "url_form" => "categorias_film.php", "url" => "categorias_film.php", "ver_ordenar" => "1");
        $r->columnas = array(
    ...
    function pre_ins() {
        $this->datos->agregar("idseccion", $this->datos->get_simple("select id from can_secciones where nombre like 'Film / Vídeo' and idnumero=" . $_SESSION["num_activo"]));
        $this->datos->agregar("plantilla", $_POST["plantilla"]);
        $this->datos->agregar("posicion", $this->datos->get_simple("select ifnull(max(a.posicion),0)+1 from can_categorias a inner join can_secciones  b where a.idseccion=b.id and b.nombre like 'Film / Vídeo' and idnumero=" . $_SESSION["num_activo"]));
    }
