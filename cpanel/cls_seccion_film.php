...
class Registro extends MantixBase {
    ...
    function formulario() {
        if($_POST["accion"]!=20 && $_POST["accion"]!=2){
            $_POST["accion"]=20;
            $_POST["idobj"]=$this->datos->get_simple("select id from can_secciones where nombre='Film / Vídeo' and idnumero=".$_SESSION["num_activo"]);
        }
        if($_POST["accion"]=="2"){
        ...
        $m_Form->controles=array(
                ...
                array("label"=>"Pie de foto/vídeo:","campo"=>"piefoto","tipo"=>"text"),
                array("label"=>"Texto<br/>(Español):","campo"=>"texto_es","tipo"=>"fck"),
                array("label"=>"Texto<br/>(Inglés):","campo"=>"texto_en","tipo"=>"fck"),
                array("label"=>"Imágen de Submenú:","campo"=>"imgseccion","tipo"=>"archivogg",
    ...
    function lista() {
        $r = new MantixGrid();
        $sql="select *, replace(replace(replace(plantilla,'plantilla','Plantilla '),4,1),5,4) as pl from can_secciones where nombre='Film / Vídeo' and idnumero=".$_SESSION["num_activo"];
        $r->atributos=array("sql"=>$sql,"nropag"=>"20","ordenar"=>"id desc","url_form"=>"seccion_film.php","url"=>"seccion_film.php","ver_eliminar"=>"0","ver_estado"=>"0","ver_nro"=>"0");
        $r->columnas=array(
    ...
    function pre_upd(){
        $this->datos->agregar("nombre",utf8_encode("Film / Vídeo"));
